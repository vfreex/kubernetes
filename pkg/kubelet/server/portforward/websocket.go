...
func NewV4Options(req *http.Request) (*V4Options, error) {
	...
	reverseForwardingStrings := req.URL.Query()[api.PortForwardReverseForwardingHeader]
	reverseForwarding := false
	if len(reverseForwardingStrings) > 0 && reverseForwardingStrings[0] == "1" {
		reverseForwarding = true
	}
...
func (h *websocketStreamHandler) portForward(p *websocketStreamPair) {
	...
	err := h.forwarder.PortForward(h.pod, h.uid, p.port, p.reverse, p.dataStream)
